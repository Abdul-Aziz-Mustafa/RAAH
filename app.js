...
app.post("/", function(req,res){
    ...
    if(pass === pass2){
        var sql = `INSERT INTO rider (R_id, Name, Email, Contact_no ,Password, location) VALUES (11, "${name}", "${mail}","${mob}","${pass}", "0000")`;
...
app.post("/book_car", function(req,res){
    ...
    // Trigger 1
    var sql = "SELECT  driver.Name, driver.Contact_no, driver.Car_type, driver.Num_plate FROM driver  INNER JOIN car ON driver.D_id = car.D_id AND car.Car_type = ? AND onride = 0";
...
app.get("/ridebook", function(req,res){
    var sql = "INSERT INTO rid_req (req_id, car_type, R_id, Date , pick_loc, drop_loc) VALUES (301, ?, ?, ?, ?, ?)"
        connection.query(sql, [loc1.car_type, loc1.R_id, loc1.Date, loc1.pick, loc1.drop], function(err, result){
            if(err) throw err;
